    ...
    const float aoa = 0.0f;                      //angle of attack
    ...
    while(lbm.get_t()<=units.t(T_Steps)) { // main simulation loop
        ...
        write_line(path + "Cd"+to_string(aoa)+".dat", to_string(lbm.get_t()) + "\t" + to_string(Cd, 3u) + "\n");
        write_line(path + "Cl"+to_string(aoa)+".dat", to_string(lbm.get_t()) + "\t" + to_string(Cl, 3u) + "\n");
        if(lbm.graphics.next_frame(T_Steps, 25.0f)) { // render enough frames for 25 seconds of 60fps video
            ...
            lbm.graphics.write_frame(get_exe_path()+"export/camera_results_0.0/"); // export image from camera position
        }
